...
@app.route('/v1/expenses/<int:expense_id>', methods= ['POST'])
def addExpense(expense_id):
        getAttribute = request.get_json(force=True)
        expense = Expenses(expense_id,getAttribute ['name'], getAttribute['email'], getAttribute['category'],
